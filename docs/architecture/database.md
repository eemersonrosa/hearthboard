...
Hearthboard stores everything in a single **SQLite** database accessed synchronously
...
| 11 | `schema11-hearthboardPhotos.js` | Adds `hearthboard_photos` (locally uploaded photos). |
...
`globalThis.__HEARTHBOARD_SCHEMA_MIGRATION_CONTEXT`, and writes the new
...
**`photo_sources`** — configured photo providers (Immich, Google Photos, Hearthboard upload).
```
...
**`hearthboard_photos`** — photos uploaded directly to Hearthboard.
```
...
- `./hearthboard/data/` — the SQLite database.
- `./hearthboard/uploads/` — user avatars, uploaded photos, and widget files.
