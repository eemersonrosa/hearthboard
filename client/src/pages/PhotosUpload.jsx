...
const PhotosUpload = () => {
  ...
  const fetchSources = async () => {
    try {
      ...
      const hearthboardSources = Array.isArray(data)
        ? data.filter((s) => s.type === 'HearthboardPhotos')
        : [];
      setSources(hearthboardSources);

      if (hearthboardSources.length === 0) {
        setSelectedSourceId(null);
        return;
      }

      const preferred = initialSourceFromUrl &&
        hearthboardSources.find((s) => s.id === initialSourceFromUrl)
        ? initialSourceFromUrl
        : hearthboardSources[0].id;
    ...
    >
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
              <PhotoLibrary fontSize="medium" /> Hearthboard Photos
            </Typography>
          </Box>
          <Button
            onClick={handleDone}
        ...
        <Typography variant="body2" sx={{ opacity: 0.8 }}>
          Upload photos from this device. They will be stored locally on your Hearthboard
        ...
        {!loading && sources.length === 0 && (
          <Alert severity="info">
            No Hearthboard Photos source is configured yet. Open the Photos widget
            settings, click "Add Source", and choose "Hearthboard Photos" to create one.
          </Alert>
        )}
