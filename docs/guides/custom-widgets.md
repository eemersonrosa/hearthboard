...
Hearthboard supports **custom widgets**: self-contained HTML files that render inside
...
- A widget is a single `.html` file uploaded via **Admin Panel → Plugins** (or
  installed from the `eemersonrosa/hearthboardPlugins` GitHub repo).
...
Because widgets are sandboxed and same-origin, they can call Hearthboard's backend —
