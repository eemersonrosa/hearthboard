...
const schemaMigrations = [
  ...
  { schemaId: 11, migrationPath: './migrations/schema11-hearthboardPhotos', },
...
const GITHUB_REPO_OWNER = 'eemersonrosa';
const GITHUB_REPO_NAME = 'HearthboardPlugins';
const GITHUB_API_BASE = 'https://api.github.com';

const DEFAULT_HEARTHBOARD_REPOSITORY = 'eemersonrosa/hearthboard';
...
const BACKEND_GITHUB_REPOSITORY = (process.env.BACKEND_GITHUB_REPOSITORY || process.env.GITHUB_REPOSITORY || DEFAULT_HEARTHBOARD_REPOSITORY).trim();
...
const ENCRYPTION_KEY = process.env.ENCRYPTION_KEY || 'hearthboard-default-key-change-in-production-32bytes';
...
fastify.get('/api/stats', async (request, reply) => {
  const repository = isValidRepositorySlug(BACKEND_GITHUB_REPOSITORY)
    ...
    : DEFAULT_HEARTHBOARD_REPOSITORY;
  ...
  try {
    ...
    const response = await axios.get(repoUrl, {
      headers: {
        ...
        'User-Agent': 'Hearthboard-Server/1.0'
      },
      timeout: 10000
    });
...
function runSchemaMigrationModule(migration) {
  globalThis.__HEARTHBOARD_SCHEMA_MIGRATION_CONTEXT = {
    db,
  ...
  try {
    ...
    delete globalThis.__HEARTHBOARD_SCHEMA_MIGRATION_CONTEXT;
  }
}
...
fastify.get('/api/calendar/ics', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT * FROM events').all();
    const calendar = ical({ name: 'Hearthboard Calendar' });
    rows.forEach((event) => {
...
fastify.get('/api/proxy', async (request, reply) => {
  ...
  try {
    ...
    const axiosConfig = {
      ...
      headers: {
        'User-Agent': 'Hearthboard-Proxy/1.0',
...
<script>try{window.opener&&window.opener.postMessage({type:'hearthboard:google-oauth',ok:${ok ? 'true' : 'false'}},'*');}catch(e){}</script>
</div></body></html>`;
  };
  try {
    if (oauthError) {
...
fastify.post('/api/photo-sources', async (request, reply) => {
  ...
  if (!['Immich', 'GooglePhotos', 'HearthboardPhotos'].includes(type)) {
    return reply.status(400).send({ error: 'Type must be Immich, GooglePhotos, or HearthboardPhotos.' });
  }
  try {
...
fastify.delete('/api/photo-sources/:id', async (request, reply) => {
  ...
  try {
    ...
    const hearthboardDir = path.join(__dirname, 'uploads', 'hearthboard-photos', String(id));
    try { fsSync.rmSync(hearthboardDir, { recursive: true, force: true }); } catch (_) { }
    db.prepare('DELETE FROM hearthboard_photos WHERE source_id = ?').run(id);
...
fastify.post('/api/photo-sources/:id/test', async (request, reply) => {
  ...
  try {
    ...
    } else if (source.type === 'HearthboardPhotos') {
      const count = db.prepare('SELECT COUNT(*) AS c FROM hearthboard_photos WHERE source_id = ?').get(source.id).c;
...
// Hearthboard Photos - list uploaded photos for a source
fastify.get('/api/photo-sources/:sourceId/uploaded', async (request, reply) => {
  ...
  try {
    ...
    if (!source || source.type !== 'HearthboardPhotos') {
      return reply.status(404).send({ error: 'Hearthboard Photos source not found' });
    }
    const rows = db.prepare(
      'SELECT id, filename, original_name, mime_type, size, uploaded_at FROM hearthboard_photos WHERE source_id = ? ORDER BY uploaded_at DESC'
    ).all(sourceId);
    return rows.map((r) => ({
      ...r,
...
// Hearthboard Photos - serve a single uploaded file
fastify.get('/api/photo-sources/:sourceId/uploaded/:photoId/file', async (request, reply) => {
  ...
  try {
    const row = db.prepare(
      'SELECT filename, mime_type FROM hearthboard_photos WHERE id = ? AND source_id = ?'
    ).get(photoId, sourceId);
    if (!row) return reply.status(404).send({ error: 'Photo not found' });
    const filePath = path.join(__dirname, 'uploads', 'hearthboard-photos', String(sourceId), row.filename);
...
// Hearthboard Photos - upload one or more photos (multipart)
fastify.post('/api/photo-sources/:sourceId/uploaded', async (request, reply) => {
  ...
  try {
    ...
    const uploadDir = path.join(__dirname, 'uploads', 'hearthboard-photos', String(sourceId));
    ...
    for await (const part of parts) {
      ...
      try {
        ...
        const info = db.prepare(
          `INSERT INTO hearthboard_photos (source_id, filename, original_name, mime_type, size) VALUES (?, ?, ?, ?, ?)`
...
// Hearthboard Photos - delete an uploaded photo
fastify.delete('/api/photo-sources/:sourceId/uploaded/:photoId', async (request, reply) => {
  const { sourceId, photoId } = request.params;
  try {
    const row = db.prepare(
      'SELECT filename FROM hearthboard_photos WHERE id = ? AND source_id = ?'
    ).get(photoId, sourceId);
    if (!row) return reply.status(404).send({ error: 'Photo not found' });
    const filePath = path.join(__dirname, 'uploads', 'hearthboard-photos', String(sourceId), row.filename);
    try { await fs.unlink(filePath); } catch (_) { }
    db.prepare('DELETE FROM hearthboard_photos WHERE id = ?').run(photoId);
    return { success: true };
  } catch (error) {
    ...
    const fetchPromises = sources.map(async (source) => {
      try {
        if (source.type === 'Immich') {
          ...
          const rows = db.prepare(
            'SELECT id, filename FROM hearthboard_photos WHERE source_id = ? ORDER BY uploaded_at DESC'
          ).all(source.id);
          const photos = rows.map((r) => ({
            id: `hearthboard-${r.id}`,
...
const start = async () => {
  try {
    ...
    if (process.env.HEARTHBOARD_DISABLE_BACKGROUND_JOBS !== '1') {
      startNightlyCronJob(); // Start the nightly chore pruning job
    } else {
      console.log('Nightly background processing disabled by HEARTHBOARD_DISABLE_BACKGROUND_JOBS=1');
    }
    ...
    if (process.env.HEARTHBOARD_DISABLE_CALENDAR_SYNC !== '1') {
      ...
      console.log('Calendar sync service disabled by HEARTHBOARD_DISABLE_CALENDAR_SYNC=1');
    }
