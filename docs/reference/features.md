...
This page explains Hearthboard's user-facing feature areas and how each maps onto the
...
- **Hearthboard uploads** — images uploaded directly (including from a phone via the
  `/photos` page), stored in `hearthboard_photos` + `server/uploads/`.
...
- Upload self-contained HTML widgets through the Admin Panel, or install from the
  `HearthboardPlugins` GitHub repo. They render in sandboxed iframes, receive the
