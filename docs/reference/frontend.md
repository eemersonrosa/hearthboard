...
- **Settings changes** propagate through custom window events
  (`hearthboard:device-settings-updated`, `hearthboard:interface-settings-updated`).
...
| Page | Role |
| --- | --- |
| `PhotosUpload.jsx` | Standalone `/photos` upload page (e.g. from a phone) that adds images to a Hearthboard photo source. |
