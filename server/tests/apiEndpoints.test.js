...
const keepTestArtifacts = process.env.HEARTHBOARD_TEST_KEEP_ARTIFACTS === '1';
...
test.before(async () => {
    ...
    serverProcess = spawn('node', ['index.js'], {
        ...
        env: {
            ...
            DB_PATH: testDbPath,
            TZ: 'UTC',
            HEARTHBOARD_DISABLE_BACKGROUND_JOBS: '1',
            HEARTHBOARD_DISABLE_CALENDAR_SYNC: '1',
            ...
            BACKEND_GITHUB_REPOSITORY: 'eemersonrosa/hearthboard',
        },
...
test('GET /api/stats returns backend build metadata', async () => {
    ...
    assert.equal(body.backend.repository, 'eemersonrosa/hearthboard');
    assert.equal(
        body.backend.commitUrl,
        'https://github.com/eemersonrosa/hearthboard/commit/1234567890abcdef1234567890abcdef12345678'
    );
});
...
test('chore schedule persists due_time and sound fields through create and patch', async () => {
    const choreRes = await api('/api/chores', {
        method: 'POST',
        body: JSON.stringify({ title: 'Take medicine', description: '', is_bonus: false }),
    });
    assert.equal(choreRes.status, 200);
    const choreId = choreRes.body.id;
...
test('chore schedule rejects an invalid due_time', async () => {
    const choreRes = await api('/api/chores', {
        method: 'POST',
        body: JSON.stringify({ title: 'Bad time chore', description: '', is_bonus: false }),
    });
    const choreId = choreRes.body.id;
